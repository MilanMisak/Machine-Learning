...
% similarity computes the similarity of 2 cases.
    ...
    %distance  = levenshtein_similarity(case1.problem, case2.problem);
    %shorter = min(size(case1.problem), size(case2.problem));
    %longer = max(size(case1.problem), size(case2.problem));
    %similarity_value = ((longer - distance) / longer) * (shorter / longer);
    
    similarity_value = similarity_value * case1.weight;
end
